...
class Database:
    ...
    async def add_reddit_scraper(self, scraper):
        await self._pool.execute("INSERT INTO reddit_webhooks "
                                 "(subreddit, webhook_id, webhook_url, guild_id, channel_id) "
                                 "VALUES ($1, $2, $3, $4, $5)",
                                 scraper.subreddit, scraper.webhook_id, scraper.webhook_url,
                                 scraper.guild_id, scraper.channel_id)

    async def remove_reddit_scraper(self, webhook_id: int):
        record = await self._pool.fetchrow("DELETE FROM reddit_webhooks WHERE id = $1 RETURNING subreddit, webhook_url, channel_id", webhook_id)
        return record['subreddit'], record['webhook_url'], record['channel_id']

    async def get_reddit_webhooks_by_guild(self, guild_id: int):
        return await self._pool.fetch("SELECT id, subreddit, webhook_id FROM reddit_webhooks WHERE guild_id = $1",
                                      guild_id)
